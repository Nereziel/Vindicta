	...
	public METHOD(init)
		...
		if (IS_MULTIPLAYER) then {
			if (! (isClass (configfile >> "CfgPatches" >> "ace_medical"))) then {
				diag_log "[Vindicta] Ace medical system is disabled, checking for Armor Plates System";
				if (! (isClass (configfile >> "CfgPatches" >> "diw_armor_plates_main"))) then {
					diag_log "[Vindicta] addon Armor Plates System is disabled, initializing standard Arma revive system";
